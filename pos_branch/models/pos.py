...
class ResBranch(models.Model):
    ...
    # For invoice fields base on branch
...
class PosSession(models.Model):
    ...
    pos_name = fields.Char(string="POS Name", readonly=True, store=True)

    @api.model
    def create(self, vals):
        res = super(PosSession, self).create(vals)
        print('res.config_id.nameres.config_id.name', res.config_id.name)
        res.write({
            'branch_id': res.config_id.branch_id.id,
            'pos_name': res.config_id.name
        })
        return res
...
class POSOrder(models.Model):
    ...
    def _process_order(self, order, draft, existing_order):
        ...
        print('pos_orderpos_orderpos_order', pos_order.pos_name)
        if pos_order:
            pos_order.write({
                'pos_name': pos_order.config_id.name,
...
class PosPayment(models.Model):
    ...
    # pos_id = fields.Many2one('pos.config', related='order_id.pos_id', store=True)
...
class AccountBankStatementLine(models.Model):
    ...
    name_arabic = fields.Char('Arabic Name', index=True)
